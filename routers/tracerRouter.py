...
def getAlumniKuisionerByInstitution(institutionId: str, institutionType: str):
    ...
    urlToGetIds = URL+ALUMNI_PORT+'/alumni/getalumnidata'
    if institutionType == '4':
        urlToGetIds = URL+INSTITUTION_PORT+'/institution/kuisioner/alumni'
    print(urlToGetIds)
    response = requests.post(urlToGetIds, data=json.dumps(alumniDataQueries), headers={
        'app-origins': 'yes',
        'Content-Type': 'application/json',
    })
    print(response)
    ...
    print(len(alumniFromInstitutionData))
...
def getConsumerKuisionerByInstitution(institutionId: str, institutionType: str):
    ...
    print(consumerFromInstitutionData)
    if len(consumerFromInstitutionData) > 0:
        idsfff = {
            'ids': consumerFromInstitutionData
        }
        consumer = requests.post(URL+ALUMNI_PORT+'/alumni/get-consumer-id', data=json.dumps(idsfff), headers={
            'app-origins': 'yes',
            'Content-Type': 'application/json',
        })
        ...
        if len(consData) > 0:
            ...
            raise HTTPException(
                status_code=400,
                detail="No Kuisioner Available Yet f",
            )
    else:
        raise HTTPException(
            status_code=400,
